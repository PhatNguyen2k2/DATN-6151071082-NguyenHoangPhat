...
public interface IEmployeeSkillRepository extends JpaRepository<EmployeeSkill, BigDecimal> {
    ...
    @Query(value = "SELECT * FROM employee_skill where skill_id = ?1 order by employee_id", nativeQuery = true)
    public List<EmployeeSkill> getAllEmployeeSkillBySkillId(BigDecimal skillId);
}
